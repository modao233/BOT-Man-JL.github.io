...
在元编程的很多应用场景中，需要对类型进行测试，即对不同的类型实现不同的功能。而常见的测试类型又分为两种：判断一个类型 **是否为特定的类型** 和 **是否满足某些条件**。前者可以通过对模板的 **特化** 直接实现；后者的测试可以通过 **测试表达式**（[sec|测试表达式]）间接完成的，即利用 C++ 11 的 `type_traits` 判断条件是否满足，生成出对应的常量表达式，再测试常量表达式。
