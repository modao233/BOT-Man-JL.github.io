...
很多人会认为：被移动后的对象会进入一个 **非法状态** _(invalid state)_，从而不能再使用。实际上，C++ 标准要求对象 **遵守移动语义**（参考 [sec|移动语义]）—— 被移动后，进入一个 **合法但未指定状态** _(“valid but unspecified” state)_ —— 即调用该对象的方法不会出现异常。处于这个状态的对象：
...
> 2019/11/27 补充：
> 
> 如果已经了解了 初始化/构造 基本概念，可以直接跳过这个部分。

变量的 **初始化** _(initialization)_ 是指 在变量创建时，为其提供 **初始值** _(initial value)_ —— 将初始值绑定到变量上，后续对变量的操作就映射为对该初始值的操作；对于不同类型的变量，有着 [不同的初始化方法](https://en.cppreference.com/w/cpp/language/initialization)。

**引用类型** 变量的初始化（一般变量/类的成员变量/函数参数/函数返回值）和其他变量的 [初始化不太一样](https://en.cppreference.com/w/cpp/language/reference_initialization)：

- 在变量创建时，**必须初始化**（C++ 的引用变量类似于是 其引用的值 的一个 **别名** _(alias)_；对引用变量的 **赋值运算** _(assignment operation)_ 是对 其引用的值 进行的 赋值运算）
- 一旦引用变量绑定了初始值，就 **不能重新绑定** 到其他值上了（而 Java/JavaScript 等 引用类型为主 的语言不一样，对引用变量赋值，会导致引用的 **重新绑定** _(rebind)_）

所以，并不是所有变量的初始化，都会导致值的 **构造** _(construction)_ —— **引用的初始化只是创建一个值的别名**。

## 右值引用

> 如果已经了解了 左右值/引用 基本概念，可以直接跳过这个部分。
...
- 右值对象一般是临时对象；转移该对象时，内部包含的资源 **不需要先复制再删除**，只需要直接 **从旧对象移动到新对象**
- 被移动的对象，**能正确析构**（参考 [sec|误解：被移动的对象不能再使用]）
